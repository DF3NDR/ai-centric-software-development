...
- **Does this project have an issue tracker — GitHub Issues, GitLab,
  Jira, a TODO.md, a `notes/` directory, any backlog artifact? If
  yes: where does it live?** *(Added in v1.1. The issue tracker is
  consulted during Step 03 for requirements derivation. Capture its
  location here so Step 03 can load it without re-discovery. If the
  Toolset Augmentation Document from step-00b already recorded this,
  confirm and move on.)*
...
*(Added in v1.1. If any findings in this artifact carry [CONFIRM],
[AWARE], or [QUESTION] flags, or are tagged at Low or Medium
confidence when High was desired, enumerate them here as explicit
verification tasks for Step 02. Step 02 will allocate time to
resolve these — either by direct measurement, practitioner
confirmation, or explicit deferral with a note in the gap register.)*

| Finding ID | Claim | Current Flag / Confidence | Verification Task for Step 02 |
|-----------|-------|--------------------------|-------------------------------|
| | | | [Run command / Consult dashboard / Paste code excerpt / Practitioner-confirm] |
...
|---------|------|--------|-------------------|
| v1.0 | 2026-04-18 | Initial authoring | Initial Step 01 prompt. |
| **v1.1** | **2026-04-20** | **Diamonds dogfooding run (obs 12, 14, 17, 10)** | Added issue-tracker presence question to the System Identification & Scope cluster (Cluster I). Added "Verification Tasks for Next Step" subsection to the artifact template — findings carrying [CONFIRM]/[AWARE]/[QUESTION] flags or Low/Medium confidence are enumerated as explicit verification tasks for Step 02, making the hypothesis-to-verification flow (Cluster H) disciplined rather than accidental. |
